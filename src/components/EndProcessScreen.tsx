...
export default function EndProcessScreen({ onBack, onNext, wallet }: EndProcessScreenProps) {
  ...
  const handleEndProcess = async () => {
    try {
      ...
      // If no wallet was provided, create one using environment variables
      if (!walletToUse) {
        ...
        walletToUse = new Wallet(privateKey.replace(/^0x/, ''), provider);
      }
      ...
      const registry = new ProcessRegistryService(registryAddress, walletToUse);
