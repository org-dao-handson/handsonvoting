...
export default function VotingScreen({ onBack, onNext }: { onBack: () => void; onNext: () => void }) {
  ...
  const castVote = async () => {
    ...
    try {
      ...
      // 5️⃣ Build & pad fieldValues
      const flat = questions.flatMap((q, i) => {
        const bits = Array(q.choices.length).fill('0');
        bits[answers[i]] = '1';
        return bits;
      });
  ...
  return (
    <Box sx={{maxWidth:600,mx:'auto',my:4}}>
      ...
      {!address ? <Alert severity="info">Connect your wallet</Alert>
        : !eligible ? <Alert severity="warning">{address} not eligible</Alert>
        : voteSubmitted ? 
        (
          <>
          ...
          <>
            {questions.map((q, i) => (
              <Box key={i} sx={{ mb: 2 }}>
                ...
                <RadioGroup value={answers[i]} onChange={e => setAnswers(prev => ({ ...prev, [i]: +e.target.value }))}>
                  {q.choices.map((c, ci) => (
                    <FormControlLabel key={ci} value={ci} control={<Radio />} label={c.title.default} />
                  ))}
                </RadioGroup>
              </Box>
            ))}
            ...
            <Button
              ...
              fullWidth
              onClick={castVote}
              disabled={activeStep >= VOTE_STEPS.length || Object.values(answers).some(v => v < 0)}
            >
